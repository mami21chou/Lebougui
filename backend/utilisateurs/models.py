...
#ici on cree un user avec BaseUserManager car on veut changer la methode de connexion de django
...
class Utilisateur(AbstractBaseUser,PermissionsMixin):    
    ...
    is_active = models.BooleanField(default=True)    # C'est ce champ qu'on mettra à False pour "suspendre" un utilisateur.
    is_staff = models.BooleanField(default=False) # True = peut se connecter à l'interface /admin de Django.     # False pour un pêcheur/acheteur/livreur normal.

    objects=UtilisateurManager()  #Branchement du manager personnalisé 

    USERNAME_FIELD="email"
    REQUIRED_FIELDS=["nom", "prenom"]     # Champs demandés uniquement quand on crée un superuser en ligne de commande
...
class ProfilLivreur(models.Model):
    """
    separer de la table utilisateur pour éviter d'avoir
    des colonnes vides/inutiles pour la plupart des utilisateurs. disponible du livreur c'est un attribut que l'acheteur n'a pas besoin
    """
...
class Premium(models.Model):
    ...
    date_obtention=models.DateTimeField(auto_now=False, auto_now_add=False, null=True,blank=True)
    date_expiration=models.DateTimeField(auto_now=False, auto_now_add=False, null=True,blank=True) 
    statut= models.CharField(choices=Statut.choices, default=Statut.EN_ATTENTE, max_length=30)
    valide_par=models.ForeignKey(Utilisateur, on_delete=models.SET_NULL, null=True, blank=True, limit_choices_to={"role": Utilisateur.Role.ADMIN}, related_name="premiums_valides" )
